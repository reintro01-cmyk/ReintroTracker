...
Deno.serve(async (req: Request) => {
  ...
  // ── Rate limit ─────────────────────────────────────────────────────────────
  const limited = await enforceRateLimit(supabase, "parse-schedule-order", RATE_LIMIT_PER_HOUR);
...
{ "order": ["Group A", "Group B", ...] }`;
  ...
  try {
    ...
    console.error("parse-schedule-order failed:", err);
    return jsonResponse({ error: "Could not parse the ordering request. Please try again." }, 500);
  }
});
